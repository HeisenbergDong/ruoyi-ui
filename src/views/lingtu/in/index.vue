<template>
  <div class="app-container">
    <el-form :model="queryParams" ref="queryForm" size="small" :inline="true" v-show="showSearch" label-width="68px">
      ...
      <el-form-item label="连续签到天数，判断用户是否连续签到" prop="days">
        <el-input
          v-model="queryParams.days"
          placeholder="请输入连续签到天数，判断用户是否连续签到"
          clearable
          @keyup.enter.native="handleQuery"
        />
      </el-form-item>
      <el-form-item label="最后一次签到天数" prop="lastTime">
        <el-date-picker clearable
          v-model="queryParams.lastTime"
          type="date"
          value-format="yyyy-MM-dd"
          placeholder="请选择最后一次签到天数">
        </el-date-picker>
      </el-form-item>
      <el-form-item>
    ...
    <el-table v-loading="loading" :data="inList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="连续签到天数，判断用户是否连续签到" align="center" prop="days" />
      <el-table-column label="最后一次签到天数" align="center" prop="lastTime" width="180">
    ...
    <el-dialog :title="title" :visible.sync="open" width="500px" append-to-body>
      <el-form ref="form" :model="form" :rules="rules" label-width="80px">
        ...
        <el-form-item label="连续签到天数，判断用户是否连续签到" prop="days">
          <el-input v-model="form.days" placeholder="请输入连续签到天数，判断用户是否连续签到" />
        </el-form-item>
        <el-form-item label="最后一次签到天数" prop="lastTime">
          <el-date-picker clearable
...
<script>
import { listIn, getIn, delIn, addIn, updateIn } from "@/api/lingtu/in";
...
export default {
  ...
  data() {
    return {
      ...
      rules: {
        ...
        days: [
          { required: true, message: "连续签到天数，判断用户是否连续签到不能为空", trigger: "blur" }
        ],
      }
    };
  },
  ...
  methods: {
    ...
    handleDelete(row) {
      ...
      this.$modal.confirm('是否确认删除签到统计编号为"' + ids + '"的数据项？').then(function() {
        return delIn(ids);
      }).then(() => {
        this.getList();
