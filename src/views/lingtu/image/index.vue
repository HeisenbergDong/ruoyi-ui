<template>
  <div class="app-container">
    ...
    <el-table v-loading="loading" :data="imageList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="商品详情图url" align="center" prop="goodsImage" width="100">
...
<script>
import { listImage, getImage, delImage, addImage, updateImage } from "@/api/lingtu/image";
...
export default {
  ...
  data() {
    return {
      ...
      rules: {
        ...
        goodsImage: [
          { required: true, message: "商品详情图url不能为空", trigger: "blur" }
        ],
      }
    };
  },
  ...
  methods: {
    ...
    handleDelete(row) {
      ...
      this.$modal.confirm('是否确认删除商品图片编号为"' + ids + '"的数据项？').then(function() {
        return delImage(ids);
      }).then(() => {
        this.getList();
